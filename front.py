...
def upload_file(file_path):
    ...
    if response.status_code == 200:
        saved_filename = response.json().get("saved_file", filename)
        return "✅ File uploaded successfully! Choose an action below.", saved_filename
...
with gr.Blocks(title="AI Document Assistant", theme=gr.themes.Soft(), css=custom_css) as demo:
    ...
    gr.Markdown("Upload a text file (.txt, .pdf, .docx) and choose to summarize, translate, or generate a quiz.")
    
    current_filename = gr.State(None)
    
    with gr.Row():
        file_input = gr.File(label="Upload File (.txt, .pdf, .docx)", file_types=[".txt", ".pdf", ".docx"])
